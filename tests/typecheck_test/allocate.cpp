...
void allocate(int count) {
    ...
    AllocType *heap = (AllocType*)calloc(10, sizeof(AllocType));
#ifndef DO_PASSING
    {
        AllocType *ptr = &(heap[1]);
        ptr = new (ptr) AllocType();
    ...
    AllocType *heap = (AllocType*)calloc(count, sizeof(AllocType));
#ifndef DO_PASSING
    {
        AllocType *ptr = &(heap[1]);
        ptr = new (ptr) AllocType();
...
#endif
#endif
#ifdef BASE_NESTED_MIXED
        if (static_cast<CAST_TYPE*>(&(ptr->secondMember)) == NULL) {
            exit(-1);
        }
#endif
        checkcast(getbaseptr(ptr));
